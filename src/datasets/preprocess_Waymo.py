...
for file_num, file in enumerate(datadirs):
    if SINGLE_TRACK_INFO_FILE:
        tracking_info = {}
    if file_num > 0 and DEBUG:
        break
    file_name = file.split("/")[-1].split(".")[0]
    print("Processing file ", file_name)
    if not os.path.isdir(os.path.join(saving_dir, file_name)):
        os.mkdir(os.path.join(saving_dir, file_name))
    if not os.path.isdir(os.path.join(saving_dir, file_name, "images")):
        os.mkdir(os.path.join(saving_dir, file_name, "images"))
    if not os.path.isdir(os.path.join(saving_dir, file_name, "point_cloud")):
        os.mkdir(os.path.join(saving_dir, file_name, "point_cloud"))
    if not SINGLE_TRACK_INFO_FILE:
        if not os.path.isdir(os.path.join(saving_dir, file_name, "tracking")):
            os.mkdir(os.path.join(saving_dir, file_name, "tracking"))
    ...
    for f_num, data in enumerate(tqdm(dataset)):
        ...
        width, height, camera_labels = (
            np.zeros([len(frame.images)]),
            np.zeros([len(frame.images)]),
            defaultdict(dict),
        )
        for im in frame.images:
            saving_name = os.path.join(
                ...
                file_name,
                "images",
                "%03d_%s.png" % (f_num, open_dataset.CameraName.Name.Name(im.name)),
            )
            if not DEBUG:
                im_array = tf.image.decode_jpeg(im.image).numpy()
                # TODO: Find best compression level
            ...
            extrinsic[im.name - 1, :, :] = np.reshape(
                frame.context.camera_calibrations[im.name - 1].extrinsic.transform,
                [4, 4],
            )
            if SAVE_INTRINSIC:
                intrinsic[im.name - 1, :] = frame.context.camera_calibrations[
                    im.name - 1
                ].intrinsic
                assert isotropic_focal(
                    waymo.read_intrinsic(intrinsic[im.name - 1, :])
                ), "Unexpected difference between f_u and f_v."
            ...
            for obj_label in frame.projected_lidar_labels[im.name - 1].labels:
                camera_labels[im.name][
                    obj_label.id.replace(
                        "_" + open_dataset.CameraName.Name.Name(im.name), ""
                    )
                ] = extract_label_fields(obj_label, 2)
        ...
        for (laser, pts) in zip(frame.lasers, points):
            saving_name = os.path.join(
                ...
                file_name,
                "point_cloud",
                "%03d_%s.ply" % (f_num, open_dataset.LaserName.Name.Name(laser.name)),
            )
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(pts)
            o3d.io.write_point_cloud(saving_name, pcd)
            laser_calib[laser.name - 1, :, :] = np.reshape(
                frame.context.laser_calibrations[laser.name - 1].extrinsic.transform,
                [4, 4],
            )

        if "intrinsic" in tracking_info:
            assert (
                np.all(tracking_info["intrinsic"] == intrinsic)
                and np.all(tracking_info["width"] == width)
                and np.all(tracking_info["height"] == height)
            )
        else:
            (
                tracking_info["intrinsic"],
                tracking_info["width"],
                tracking_info["height"],
            ) = (intrinsic, width, height)
        dict_2_save = {
            "per_cam_veh_pose": pose,
            "cam2veh": extrinsic,
            "im_paths": im_paths,
            "width": width,
            "height": height,
            "veh2laser": laser_calib,
        }
        ...
        if SINGLE_TRACK_INFO_FILE:
            ...
            with open(
                os.path.join(saving_dir, file_name, "tracking", "%03d.pkl" % (f_num)),
                "wb",
            ) as f:
    ...
    if SINGLE_TRACK_INFO_FILE:
        with open(
            os.path.join(
                saving_dir,
                file_name,
                "tracking_info%s.pkl" % ("_debug" if DEBUG else ""),
            ),
            "wb",
        ) as f:
