...
def get_dataset(datadir, scene_dict, args=None):
    ...
    if scene_dict["type"] == "vkitti":
        scene_id = scene_dict["scene_id"]
        datadir_tmp = "/mnt/public/datasets/vkitti2"
        if os.path.exists(datadir_tmp):
            datadir = datadir_tmp
        datadir_full = os.path.join(datadir, f"Scene0{scene_id}/clone")
        dataset = VKitti(datadir_full, scene_dict)

    elif scene_dict["type"] == "kitti":
        scene_id = scene_dict["scene_id"]
        datadir_full = os.path.join(datadir, "%04d"%(scene_id))
        dataset = Kitti(datadir_full, scene_dict, selected_frames=selected_frames)

    elif scene_dict["type"] == "waymo":
        from .waymo import Waymo

        # If on toolkit machine
        datadir_tmp = "/mnt/public/waymo/validation"
        if os.path.exists(datadir_tmp):
            datadir = datadir_tmp
        ...
        subdirs_list = [
            d.name for d in os.scandir(datadir) if d.is_dir() if d.name[-4:].isnumeric()
        ]
        assert scene_id[0] < len(
            subdirs_list
        ), "Folder %s does not have %d subfolers" % (datadir, scene_id[0] + 1)
        ...
        for sub_dir in sorted(subdirs_list):
            if sub_dir[-4:].isnumeric():
                if int(sub_dir[-4:]) == scene_id[0]:
                    ...
                    datadir_full = sorted(
                        [f.path for f in os.scandir(datadir_full) if f.is_dir()]
                    )[record_id]
                    break
